...
// 		console.log("READY");
// 		console.log($);
// 			$('#shipping_postcode').parsley().on('field:success', function() {
// 			console.log('validate shipping postcode')
...
(function($){
  ...
  window.selectPostcode = async function (postcode) {
    ...
    setTimeout(postcodeInput.dispatchEvent(new Event("input")), 100)

    try {
      const response = await $.ajax({
        url: `https://api.ideal-postcodes.co.uk/v1/postcodes/${postcode}?api_key=ak_m867c38yUg1QlKQwkfPZNa0oMdhWx`,
        type: 'GET',
      });
